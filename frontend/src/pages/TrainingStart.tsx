...
import { api, type ModeStats, type QuestionTypeMode, type StatsResponse, type TrainingDirectionMode, type TrainingMode, type TrainingSettingsSnapshot } from "@/lib/api";
...
export default function TrainingStart() {
  ...
  const [questionTypeMode, setQuestionTypeMode] = useState<QuestionTypeMode>("default");
  ...
  useEffect(() => {
    ...
    Promise.all([api.getStats(), api.getSettings()]).then(([s, set]) => {
      ...
      setSettings({
        ...
        contextOptionsCount: set.contextOptionsCount,
  ...
  const start = async () => {
    try {
      const session = questionTypeMode === "context_cloze"
        ? await api.createContextTraining(mode, override ? settings : undefined)
        : await api.createTraining(mode, override ? settings : undefined);
      navigate(`/training/${session.sessionId}`);
    } catch (e) { toast.error(e instanceof Error ? e.message : "Не удалось начать тренировку"); }
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <Card className="border-border/50"><CardHeader><CardTitle className="text-base">Настройки</CardTitle><CardDescription className="text-xs">Можно использовать глобальные настройки или переопределить только для этой сессии.</CardDescription></CardHeader><CardContent className="space-y-4">
        <div className="space-y-1.5"><Label>Тип упражнения</Label><Select value={questionTypeMode} onValueChange={(value: QuestionTypeMode) => setQuestionTypeMode(value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="default">Обычные карточки</SelectItem><SelectItem value="context_cloze">Предложение с пропуском</SelectItem></SelectContent></Select></div>
        ...
        <fieldset disabled={!override} className="grid gap-4 sm:grid-cols-2 disabled:opacity-60">
          ...
          {questionTypeMode === "context_cloze" && <div className="space-y-1.5"><Label>Вариантов для предложений</Label><Input type="number" min={2} max={8} value={settings.contextOptionsCount} onChange={(e) => update("contextOptionsCount", Number(e.target.value))} /></div>}
        </fieldset>
